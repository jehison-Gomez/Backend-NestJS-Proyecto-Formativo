...
export class TypeOrmRolRepository implements RolRepository {
  ...
  private toDomain(orm: RolOrmEntity): Rol {
    return new Rol({
      ...
      activo: orm.activo,
      id_usuario: (orm as any).usuario?.id_usuario,
    });
  }
  ...
  private toOrm(rol: Partial<Rol>): Partial<RolOrmEntity> {
    return {
      ...
      ...(rol.id_usuario !== undefined && { usuario: { id_usuario: rol.id_usuario } as UsuarioOrmEntity }),
    };
  }
  ...
  async create(rol: Rol): Promise<Rol> {
    ...
    const reloaded = await this.repo.findOne({ where: { id_rol: saved.id_rol }, relations: RELATIONS });
  ...
  async findOne(id: string): Promise<Rol | null> {
    const found = await this.repo.findOne({ where: { id_rol: id }, relations: RELATIONS });
    return found ? this.toDomain(found) : null;
  }

  async update(id: string, rol: Partial<Rol>): Promise<Rol> {
    const existing = await this.repo.findOne({ where: { id_rol: id }, relations: RELATIONS });
